...
class InvoiceForm extends React.Component {
  constructor(props) {
    ...
    this.isEditing = window.location.pathname.includes('edit');

    if (this.isEditing) {
      const editInvoiceNumber = window.location.pathname.split('/').pop();
      const editInvoiceItem = props.invoice.filter(item => item.invoiceNumber == editInvoiceNumber)[0];
  ...
  handleCalculateTotal() {
    ...
    items.forEach((item) => {
      let itemTotal = parseFloat(item.price).toFixed(2) * parseInt(item.quantity).toFixed(2);
      subTotal += itemTotal;
    });
  ...
  invoiceNumberValidate = (event) => {
    this.props.invoice.some(item => item.invoiceNumber === event.target.value) ?
      toast.error(`Invoice number ${event.target.value} already exist`)
  ...
  onCurrencyChange = (selectedOption) => this.setState(selectedOption);
                ...
                <div className="d-flex flex-row align-items-start justify-content-between mb-3">
                  ...
                  <div className="d-flex flex-row align-items-center">
                    ...
                    <Form.Control
                      ...
                      name={"invoiceNumber"}
                      onChange={(event) => this.invoiceNumberValidate(event)}
                      min="1"
                      style={{ maxWidth: '70px' }}
                      required="required"
                ...
                <Row className="mb-5">
                  <Col>
                    ...
                    <Form.Control
                      ...
                      type="text" name="billToAddress"
                      className="my-2" autoComplete="address"
                      onChange={(event) => this.editField(event)}
                      required="required"
                    />
...
const mapDispatchToProps = (dispatch) => ({
  ...
  editInvoice: (data) => dispatch(editInvoice(data)),
});
